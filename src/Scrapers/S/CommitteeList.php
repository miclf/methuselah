...
class CommitteeList extends AbstractScraper
{
    ...
    protected $types = [
        // French
        "commission mixte"      => 'mixed',
        "commission spéciale"   => 'special',
        "commission permanente" => 'permanent',

        // Dutch
        "gemengde commissie"   => 'mixed',
        "bijzondere commissie" => 'special',
        "vaste commissie"      => 'permanent',
    ];
    ...
    {
        return ['s.committee_list', ['lang' => $lang]];
    }
    ...
    protected function scrapeCommitteeList($lang)
    {
        ...
        $this->getNodes($lang)->each(function ($node) use (&$type, $lang) {
            ...
            $matches = $this->match('#COMID=(\d+)#', $node->attr('href'));
    ...
    public function getNodes($lang)
    {
        ...
        return $this->getCrawler($document)->filter('h2, li a');
    }
